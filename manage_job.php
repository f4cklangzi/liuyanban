	...
	if (@$_GET['action']=='del' && isset($_GET['id'])) {
		...
		if ($_own_id!=$_id) {
			...
			if (_mysql_affected_rows()==1) {
				...
				_close();
				//跳转
				_location('用户已辞职！','member.php');
			}else{
				_alert_back('很遗憾,辞职失败！');
			}
		}
	}
...
?>
<script type="text/javascript" src="js/manage_job.js"></script>
</head>
<body>
<?php
				...
				<?php while (!!$_rows=_fetch_array_list($_result)) { ?>
					<tr class="line">
						...
						<td>
							...
							<a style="color: red;" name="del" href="?action=del&id=<?php echo $_rows['tg_id'] ?>">辞职</a>
						</td>
					</tr>
				<?php 
					} 
